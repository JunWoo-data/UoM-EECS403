...
string getMode(int argc, char * argv[]) {
    ...
    option long_options[] = {
        ...
        // the "help" ('h') options.
        // example) ./project0 -m nosize (-m is option and nosize is argument for -m option)
        // example) ./project0 -h (h means help and help has no argument )
    ...
    // "m" is followed single colone because '-m' require argument
    // "h" is not followed by any colone because '-h' does not require any argument
...
// cf) About resize and reserve, vectors
// vector<double> data (back in main())
// new vector, size 0, capacity 0
// size: what's in it right now
// capacity: how big is dynamic memory behind the scenes
// I can't understand...
